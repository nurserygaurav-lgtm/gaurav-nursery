...
import { ChevronRight, Leaf, RefreshCw, ShieldCheck, ShoppingBag, Star, Truck } from 'lucide-react';
...
const categories = [
  ...
  { name: 'Fertilizers', image: 'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?auto=format&fit=crop&w=600&q=80' }
];
...
export default function HomeTreeland() {
  ...
  return (
    <div className="bg-[#f7faf6] text-[#142217]">
      <section className="relative min-h-[34rem] overflow-hidden bg-[#0b3d1e]">
        <img className="absolute inset-0 h-full w-full object-cover opacity-55" src={heroImage} alt="Lush indoor plants from Gaurav Nursery" fetchPriority="high" />
        <div className="absolute inset-0 bg-[#092d17]/72" />
        <div className="premium-container relative flex min-h-[34rem] items-center py-14 sm:py-20">
          <div className="max-w-2xl text-white">
            <span className="inline-flex items-center gap-2 rounded-md border border-white/25 bg-white/10 px-3 py-2 text-xs font-black uppercase tracking-[0.16em] backdrop-blur">
              <Leaf size={15} /> Trusted Plant Studio
            </span>
            <h1 className="mt-6 font-serif text-[clamp(2.6rem,5vw,5.2rem)] font-black leading-[1.02]">Bring nature into your home.</h1>
            <p className="mt-5 max-w-xl text-base font-semibold leading-8 text-white/85 sm:text-lg">Fresh, healthy, handpicked indoor plants delivered from Gaurav Nursery to your doorstep.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-[#32d675] px-6 text-sm font-black text-[#082111] shadow-button transition hover:bg-[#73e89e]" to="/shop">
                <ShoppingBag size={18} /> Explore Shop
              </Link>
              <Link className="inline-flex min-h-12 items-center justify-center rounded-md border border-white/35 bg-white/10 px-6 text-sm font-black text-white transition hover:bg-white/20" to="/shop?sort=Popular">
                View Today&apos;s Offers
              </Link>
            </div>
          </div>
      ...
      <section className="border-b border-[#dce7da] bg-white py-5">
        <div className="premium-container grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {trustPoints.map((item) => (
            <div key={item.title} className="flex items-center gap-3">
              <item.icon className="shrink-0 text-[#16864f]" size={25} />
              <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {categories.map((category) => (
            <Link key={category.name} className="group overflow-hidden rounded-lg border border-[#dce7da] bg-white transition hover:-translate-y-0.5 hover:shadow-soft" to={`/shop?category=${encodeURIComponent(category.name)}`}>
              ...
              <p className="p-3 text-center text-sm font-black text-[#142217]">{category.name}</p>
            </Link>
          ))}
        </div>
      </section>

      <section className="premium-container pb-14 sm:pb-20">
        ...
        <div className="mt-10 flex items-center justify-center gap-2 text-sm font-bold text-slate-500"><Star className="text-[#d79322]" size={17} fill="currentColor" /> Nursery-grown quality, delivered with care.</div>
      </section>
    </div>
  );
}
