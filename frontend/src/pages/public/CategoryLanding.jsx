  ...
  return (
    <section>
      <div className="relative overflow-hidden bg-leaf-950">
        ...
        <div className="premium-container relative grid min-h-[22rem] items-end py-10 text-white">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-5 text-4xl font-black tracking-tight sm:text-5xl">{menu.label}</h1>
            <p className="mt-4 max-w-2xl text-lg font-semibold text-white/80">{menu.tagline}</p>
            <Link className="mt-7 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-black text-leaf-950 shadow-button transition hover:-translate-y-0.5 hover:bg-leaf-50" to={menu.shopAllTo}>
              Shop All {menu.label}
              <ArrowRight size={17} />
            </Link>
          </div>
      ...
      <div className="premium-container py-10">
        <div className="mb-7 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-black text-leaf-950">Explore {menu.label}</h2>
          </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {menu.sections.map((section) => (
              <article key={section.title} className="rounded-2xl border border-leaf-100 bg-white p-5 shadow-soft">
            ...
            <div className="grid gap-3">
              {menu.featured.map((product) => (
                <Link key={product.name} className="group overflow-hidden rounded-2xl border border-leaf-100 bg-leaf-50 transition hover:-translate-y-0.5 hover:shadow-soft" to={menu.shopAllTo}>
                  <img className="h-36 w-full object-cover transition duration-500 group-hover:scale-105" src={product.image} alt={product.name} loading="lazy" decoding="async" />
                  <div className="p-3">
