...
export default function CartItem({ item, isUpdating, onRemove, onQuantityChange }) {
  ...
  if (!productId) {
    return (
      <article className="rounded-lg bg-white p-4 shadow-soft">
        <p className="font-bold text-leaf-900">This cart item is unavailable.</p>
        <p className="mt-1 text-sm text-stone-600">It may have been removed or is still syncing.</p>
      </article>
    );
  }
  ...
  return (
    <article className="grid gap-4 rounded-lg bg-white p-4 shadow-soft sm:grid-cols-[96px_minmax(0,1fr)_auto] sm:items-center">
      ...
      <div>
        <Link to={`/products/${productId}`} className="block line-clamp-2 break-words font-bold text-leaf-900">
      ...
      <div className="flex items-center justify-between gap-3 sm:flex-col sm:items-end">
        <QuantityControl disabled={isUpdating} max={product.stock} onChange={(quantity) => onQuantityChange(productId, quantity)} value={item.quantity} />
        <button className="rounded-lg p-2 text-red-600 hover:bg-red-50" disabled={isUpdating} onClick={() => onRemove(productId)} type="button" aria-label="Remove item">
          <Trash2 size={18} />
        </button>
      </div>
    </article>
